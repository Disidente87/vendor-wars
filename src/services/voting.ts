...
export class VotingService {
  ...
  static async registerVote(voteData: VoteData): Promise<VoteResult> {
    try {
      ...
      const canVoteForVendor = await rateLimiter.canVoteForVendor(userFid, vendorId)
      if (!canVoteForVendor) {
        return {
          success: false,
          tokensEarned: 0,
          ...
          streakBonus: 0,
          territoryBonus: 0,
          error: 'Daily vote limit reached for this vendor (max 3 votes per day)'
        }
      }

      const canVoteThisWeek = await rateLimiter.canVoteThisWeek(userFid)
      if (!canVoteThisWeek) {
        return {
          success: false,
          tokensEarned: 0,
          ...
          streakBonus: 0,
          territoryBonus: 0,
          error: 'Weekly vote limit reached (max 200 votes per week)'
        }
      }
      ...
      const { data: existingVote, error: checkError } = await supabase
        ...
        .eq('battle_id', DEFAULT_BATTLE_ID)
        .single()
      ...
      await rateLimiter.incrementVoteCount(userFid, vendorId)
      await rateLimiter.incrementWeeklyVoteCount(userFid)
  ...
  static async calculateTokens(userFid: string, vendorId: string, voteType: 'regular' | 'verified'): Promise<TokenCalculation> {
    ...
    return {
      ...
      streakBonus,
      territoryBonus,
      totalTokens: Math.min(totalTokens, weeklyCapRemaining),
      weeklyCapRemaining
    }
  }
