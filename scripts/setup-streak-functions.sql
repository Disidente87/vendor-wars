...
-- Índice para optimizar las consultas de streak por usuario
-- NOTA: No podemos usar DATE() en índices, pero podemos indexar por usuario y timestamp
CREATE INDEX IF NOT EXISTS idx_votes_voter_created ON votes(voter_fid, created_at);

-- Índice para optimizar las consultas por usuario
CREATE INDEX IF NOT EXISTS idx_votes_voter ON votes(voter_fid);

-- Índice para optimizar las consultas por fecha (usando timestamp completo)
